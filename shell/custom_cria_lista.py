...
def cria_lista_shell():
    ...
    if [ `find /tmp/{2}_{0}.{1} | wc -l` -ge 1 ]
        ...
        for V_LISTA in `find /tmp/{2}_{0}.{1} | cut -c6-10`
            do
                ...
                mysql --user=root --password={3} iogurte < /usr/local/bin/listawork_{0}.sql >  /var/log/contact/listawork_{0}.log 2> /var/log/contact/listawork{0}.log
    ...
    '''.format(name_mailling, file_extension, num_formatting, password)
